...
/**
  ...
  *
  * The bundle is written into `Compile / resourceManaged / "public"` rather than a checked-in
  * `public/` folder. That single choice does most of the work: the output lands on the classpath as
  * `/public`, which is where `app.controllers.UIController` reads it from via
  * `assets.at("/public", ...)`; anything consuming `Compile / resources` (`package`, `dist`,
  * `stage`, docker) picks it up without each being patched individually; `clean` removes it; and it
  * never needs a .gitignore entry. It also keeps the output under `target/out`, which sbt 2
  * requires of a cached task's outputs -- see `frontendBuild`.
  *
  * This plugin builds the bundle and stops there. It deliberately does not run the Vite dev server:
  * `sbt run` starts Play and nothing else, and Play serves the bundle from the classpath on its own
  * port, exactly as it does in production.
  *
  * Note what that does *not* give you: a live UI loop. `run` builds the bundle at startup and Play
  * then serves that snapshot for the lifetime of the process. Editing a component and refreshing
  * does not show the change -- Play's dev asset classloader is fixed when `run` starts, so even
  * after its reloader reruns this task and a fresh bundle is on disk, the old one keeps being
  * served. Restarting `run` picks it up. For UI work, run `npm run dev` instead: HMR on its own
  * port, with vite.config.ts proxying /api back to Play. README.md documents both loops.
  *
  * The reason for building it here at all is that dev and prod then serve the same bytes down the
  * same code path. When the dev server fronted the app, Play's `Assets` never ran in dev, so an
  * asset-caching bug in `UIController` stayed invisible until a production build was staged -- dev
  * reported `no-cache` for everything and looked fine.
  *
  * It also means sbt owns no child process. An earlier version started the dev server from a
  * PlayRunHook, which meant reaping an npm -> node process tree on every exit path; a stale watcher
  * outliving `sbt run` holds its port and file watches. Not starting it makes that unreachable
  * rather than handled.
  *
  * The related trap, should anyone reintroduce a watcher: nothing may write into `frontendTarget`
  * except `frontendBuild`. That directory is a declared cached output, and sbt's action cache
  * assumes a declared output directory has exactly one writer -- on a hit it restores only
  * *missing* files and otherwise trusts what is on disk. A `vite build --watch` pointed there
  * leaves development bundles sitting where `dist` will ship them as the production artifact, with
  * a green build.
  */
object FrontendPlugin extends AutoPlugin {
  ...
  object autoImport {
    ...
    /**
      ...
      * `--outDir <dir> --emptyOutDir`, so the command must end in a position where those reach the
      * bundler.
      *
      * For an `npm run` script that means a trailing `--`: without it npm swallows `--outDir` as
      * its own config ("Unknown cli config") and forwards the bare path to the script, where vite
      * reads it as its positional `root` and fails to resolve an entry module.
      */
  ...
  private def sourcesTask = Def.task {
    val conv = fileConverter.value
    (frontendBuild / allInputFiles).value.map(p => conv.toVirtualFile(p): HashedVirtualFileRef)
  }
  ...
  private def installTask = Def.task {
    ...
    if (!modules.isDirectory() || !stamp.exists() || IO.read(stamp) != current) {
  ...
  /**
    * Cached on the content hashes of `frontendSources`, so an unchanged UI never reruns vite.
    *
    * `declareOutputDirectory` hands the whole bundle to sbt's action cache: it is zipped into the
    * CAS with a manifest of per-file hashes and restored by `syncBlobs` on a hit. That survives
    * `clean`, and with a remote cache configured a CI machine can restore a bundle it never built.
    * A hand-rolled stamp file can do neither -- it can only decide not to redo work whose output
    * still happens to be sitting on this disk.
    */
  ...
  /**
    * Adapts the cached bundle back to the `Seq[File]` that resourceGenerators still expects.
    *
    * The existence check is not paranoia. On a cache hit sbt only re-syncs the bundle's
    * `.sbtdir.zip` archive, and `syncFile` returns without unpacking when that archive is already
    * in place with a matching digest -- the *contents* of the directory are never validated
    * (ActionCacheStore.scala, `afterFileWrite` only fires on a rewrite). So a hand-deleted
    * frontendTarget stays deleted through any number of green builds, and without this check
    * `packageBin` ships a jar with no UI in it (observed, not theorized).
    *
    * Recovery is cheap because `unpackageDirZip` runs `putBlob` on every file it extracts: each
    * bundle file sits individually in the CAS, so missing files are restored with `syncBlobs` -- no
    * vite run, no `clean`. The error remains only for the case where the CAS itself no longer has
    * the blobs.
    */
  ...
  /**
    * Vite will not empty an outDir outside its own project root unless told to, and it must be
    * emptied so bundles dropped by a previous build do not linger on the classpath.
    */
  ...
  /**
    ...
    *
    * stderr maps to warn, not error: npm writes notices and progress there on perfectly successful
    * runs, and error-level lines in a green build train people to ignore error-level lines. Failure
    * is signalled by the exit code, which fails the task with the command attached.
    */
  private def run(
      cmd: Seq[String],
      cwd: File,
